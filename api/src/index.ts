// src/index.ts
// SaaS予約（CSVなし） + 署名検証 + 管理者限定 + RateLimit + /copy-slots + /report + /list(月) + /whoami
...
// RateLimit (per uid, fixed TTL to window end)
...
function parseReserve(text: string, defaultService = "カット"): Parsed | null {
  ...
  const time = String(m[4]).padStart(2, "0") + ":" + String(m[5]).padStart(2, "0");
  const service = (m[6] && m[6].trim()) ? m[6].trim() : defaultService;
...
async function handleSetSlots(env: Env, args: string[], replyToken: string) {
  ...
  if (!date) return lineReply(env, replyToken, "日付の形式が変です (例: 2025-10-05)");
  const times = parseTimesFlexible(args.slice(1));
  if (!times.length) return lineReply(env, replyToken, "時刻の指定が見つかりません (例: 10:00 10:30 11:00)");
  await env.LINE_BOOKING.put(K_SLOTS(date), JSON.stringify(times));
  return lineReply(env, replyToken, "OK: " + date + " の枠を更新しました\n" + times.join(", "));
}
...
async function handleSlots(env: Env, args: string[], replyToken: string) {
  ...
  if (!slots.length) return lineReply(env, replyToken, "[WARN] " + date + " の枠は未設定です。/set-slots で登録してください。");
...
async function handleReserve(env: Env, z: string, replyToken: string, userId: string, userName?: string) {
  ...
  const date = p.date, time = p.time, service = p.service;
  if (isPast(date, time)) return lineReply(env, replyToken, "過去の時間は予約できません。");

  const slotStr = await env.LINE_BOOKING.get(K_SLOTS(date));
  const slots: string[] = slotStr ? JSON.parse(slotStr) : [];
  if (!slots.includes(time)) return lineReply(env, replyToken, "その時間は枠にありません。まず /slots " + date + " で確認してください。");

  const key = date + " " + time;
  try {
    await acquire(env, key, 15);
    if (await env.LINE_BOOKING.get(K_RES(date, time))) {
      return lineReply(env, replyToken, "その枠はちょうど埋まりました。他の時間を試してください。");
    }
    ...
    return lineReply(env, replyToken, "予約を登録しました。\n日時: " + date + " " + time + "\n内容: " + service);
  } catch (e: any) {
    if (e && e.message === "LOCKED") return lineReply(env, replyToken, "今、予約が集中しています。少し時間をおいて再実行してください。");
    await notifySlack(env, "RESERVE_FAIL", { date, time, userId, err: (e && e.message) || String(e) });
    throw e;
  } finally {
...
async function handleCancel(env: Env, args: string[], replyToken: string, userId: string) {
  ...
  if (!recStr) return lineReply(env, replyToken, "その枠の予約は見つかりません。");
  const rec = JSON.parse(recStr);
  if (rec.userId !== userId) return lineReply(env, replyToken, "この予約はあなたのものではありません。");

  await env.LINE_BOOKING.delete(K_RES(date, time));
  await env.LINE_BOOKING.delete(K_USER(userId, date, time));
  return lineReply(env, replyToken, "予約をキャンセルしました。\n日時: " + date + " " + time);
}

// ===== 月別一覧（登録/予約/空き｜先頭の空き） =====
function daysInMonth(y: number, m: number): number { return new Date(y, m, 0).getDate(); }
function dowLabel(y: number, m: number, d: number): string {
...
async function listMonth(env: Env, ym: string, replyToken: string) {
  ...
  if (!yy || !mm || mm < 1 || mm > 12) {
    return lineReply(env, replyToken, "形式: /list YYYY-MM (例: /list 2025-10)");
  }
  ...
  for (let d = 1; d <= last; d++) {
    ...
    const dow = dowLabel(yy, mm, d);
    lines.push(dow + " " + date + " | " + total + "/" + reserved + "/" + free + (firstFree ? " | -> " + firstFree : ""));
  }

  return lineReply(env, replyToken, [header].concat(lines).join("\n"));
}
...
async function handleList(env: Env, args: string[], replyToken: string) {
  ...
  if (!date) return lineReply(env, replyToken, "日付の形式が変です (例: 2025-10-05 または 2025-10)");
...
async function handleCopySlots(env: Env, args: string[], replyToken: string) {
  ...
  const src = normalizeDateArg(args[0]);
  const dst = normalizeDateArg(args[1]);
  if (!src || !dst) return lineReply(env, replyToken, "日付の形式が変です (例: 2025-10-05)");
  const s = await env.LINE_BOOKING.get(K_SLOTS(src));
  const slots: string[] = s ? JSON.parse(s) : [];
  const set = Array.from(new Set(slots)).sort();
  await env.LINE_BOOKING.put(K_SLOTS(dst), JSON.stringify(set));
  return lineReply(env, replyToken, "枠をコピーしました。\n" + src + " -> " + dst + "\n" + set.join(", "));
}
...
async function handleReport(env: Env, args: string[], replyToken: string) {
  ...
  if (!ym) return lineReply(env, replyToken, "月の形式が変です (例: 2025-10)");
  ...
  for (const k of it.keys) {
    ...
    const recStr = await env.LINE_BOOKING.get(k.name);
    if (!recStr) continue;
    try {
      const rec = JSON.parse(recStr);
  ...
  return lineReply(env, replyToken, "【" + ym + " レポート】合計 " + total + "件\n— 日別 —\n" + days + "\n— サービス別 —\n" + svc);
}
...
async function whoAmI(ev: any, env: Env): Promise<string> {
  const src = ev && ev.source ? ev.source : {};
  const uid: string | undefined = src.userId;
  const gid: string | undefined = src.groupId;
  const rid: string | undefined = src.roomId;

  if (!uid) return "whoami: userId が取得できません。";
  if (!env.LINE_CHANNEL_ACCESS_TOKEN) return "whoami: トークン未設定です。wrangler secret put LINE_CHANNEL_ACCESS_TOKEN を実行してください。";
  ...
  const out = [
    ...
    prof && prof.displayName ? "name: " + prof.displayName : undefined,
    prof && prof.language ? "lang: " + prof.language : undefined,
    prof && prof.statusMessage ? "status: " + prof.statusMessage : undefined,
  ].filter(Boolean).join("\n");

  return prof ? out : out + "\n(プロフィール取得に失敗。友だち関係/権限/種別を確認してください)";
}

// ===== Router (Module Worker) =====
export default {
    ...
    try {
      ...
      if (url.pathname === "/api/line/webhook" && req.method === "POST") {
        ...
        for (const ev of events) {
          ...
          const messageText: string | undefined = ev.message && ev.message.text;
          const userId: string | undefined = ev.source && ev.source.userId;
          const userName: string | undefined = ev.source && ev.source.userId; // 実運用はプロフィールAPIへ
          if (!replyToken || !messageText || !userId) continue;

          if (!(await rateLimit(env, userId))) {
            await lineReply(env, replyToken, "リクエストが多すぎます。少し待ってから試してください。");
            continue;
          }
          ...
          try {
            if (cmd === "/set-slots" || cmd === "set-slots") {
              if (!isAdmin(userId, env)) { await lineReply(env, replyToken, "このコマンドは管理者専用です。"); continue; }
            ...
            await notifySlack(env, "WEBHOOK_CMD_FAIL", { cmd, err: (e && e.message) || String(e) });
            await lineReply(env, replyToken, "内部エラーの可能性があります。もう一度試してください。");
          }
        }
        return new Response("OK");
      }
      ...
      await notifySlack(env, "UNCAUGHT_FETCH_ERROR", { url: (req as any).url, err: (e && e.message) || String(e) });
