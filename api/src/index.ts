...
async function sendText(env: Env, e: LineEvent, text: string) {
  ...
  if (e.replyToken) {
    ...
    // 400 Invalid reply token → push fallback
...
function normSpaces(s: string) { return s.replace(/\u3000/g," ").replace(/\s+/g," ").trim(); }
...
function normalizeDateArg(arg: string): string | null {
  ...
  let m = arg.match(/^(\d{4})[\/-](\d{1,2})[\/-](\d{1,2})$/); if (m) return `${m[1]}-${z2(+m[2])}-${z2(+m[3])}`;
  m = arg.match(/^(\d{1,2})[\/-](\d{1,2})$/); if (m) return `${y}-${z2(+m[1])}-${z2(+m[2])}`;
  return null;
}
function parseTimes(csv: string){return csv.replace(/、/g,",").replace(/\s/g,"").split(",").filter(Boolean);}
...
async function listAvailable(env: Env, date: string) {
  ...
  const checks = await Promise.all(slots.map(async t => ({ t, v: await env.LINE_BOOKING.get(keyRes(date,t)) })));
...
async function handleMessage(env: Env, e: LineEvent, ctx: ExecutionContext) {
  ...
  if (cmd === "/set-slots") {
    ...
    const csv  = rest.join(" ");
    if (!date || !csv) { ctx.waitUntil(sendText(env,e,"使い方: /set-slots 2025-09-25 10:00,11:30,14:00")); return; }
  ...
  if (cmd === "/slots") {
    const [dateArg] = args;
    const date = dateArg ? normalizeDateArg(dateArg) : null;
    if (!date) { ctx.waitUntil(sendText(env,e,"使い方: /slots 2025-09-25（9/25でもOK）")); return; }
    const data = await listAvailable(env, date);
    log("slots", date, data);
    if (data.slots.length === 0) { ctx.waitUntil(sendText(env,e,`${date} の枠はまだ登録されてないよ`)); return; }
  ...
  if (cmd === "/reserve") {
    ...
    if (!date || !time) { ctx.waitUntil(sendText(env,e,"使い方: /reserve 2025-09-25 10:00 カット")); return; }
    ...
    if (exist) {
      ...
      ctx.waitUntil(sendText(env,e,`⚠️ その日時は既に予約があります。\nID: ${j.id}\n日時: ${date} ${time}\n内容: ${j.content}\n\n別の時間で予約してね🙏`));
      return;
    }
...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (req.method==="GET" && url.pathname==="/__debug/slots") {
      ...
      return new Response(JSON.stringify({ date, ...data }, null, 2), { status:200, headers:{ "Content-Type":"application/json" }});
    }
